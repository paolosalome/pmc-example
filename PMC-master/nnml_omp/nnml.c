...
*/
    ...
    nupl=makevect(NumHL+2,sizeof(int)); // vector of (num hidden layer + 2) elements: vec[i] -> # of sigmoid neuron of layer i
    ...
    if(NumHL) { // If there are hidden layers
      ...
      for(i=1; i<=NumHL; i++) { // for all hidden layers
        ...
        DeltaH2H[i-1]=(REAL **)makematr(NumPattern, nupl[i]+1,sizeof(REAL)); // with #of units (sigmoid neurons) in layer i-1 colums
      }
      for(i=1; i<=NumHL; i++) {
    ...
    for(i = 0 ; i < nthreads ; i++){
        ...
        for(h=1; h<=NumHL; h++){
          threadDeltaH2H[i][h-1] = (REAL *)makevect((nupl[h]+1)*(nupl[h-1]+1),sizeof(REAL));
        }
    }
    ...
    DeltaWeightHO=(REAL **)makematr(NumOutput+1, nupl[NumHL]+1,sizeof(REAL));
...
/** -----------------------------------------------INITIALIZATIONS--------------------------------------------------------------------------- **/
    ...
    /** maybe it's okay to change the index order (chache misses)!!! **/
    ...
    if(strcmp(RestartFileName,NULLFILE)) { // condition satisfied if strcmp != 0
        ...
        for( k = 1 ; k <= nupl[NumHL+1] ; k ++ ) {
                ...
                if(fpd) fscanf(fpd,formatstring,&DeltaWeightHO[k][0]);
                for( j = 1 ; j <= nupl[NumHL] ; j++ ) {
                    fscanf(fp,formatstring,&WeightHO[j][k]);
                    if(fpd) fscanf(fpd,formatstring,&DeltaWeightHO[k][j]);
                }
                fscanf(fp,"\n");
                if(fpd) fscanf(fpd,"\n");
        }
    ...
    /** ---------------------------------END RANDOMIZATION--------------------------------------**/
        ...
        #pragma omp parallel private(np,p,i,j,h,k)
        {
          ...
          for( np = id ; np < NumPattern ; np+=nthreads ) {
              ...
              for( j = 1 ; j <= nupl[1]; j++ ) {    /* compute hidden unit activations */
                  ...
                  Sum[p][j] = temp;
                  H2H[0][p][j] = 1.0/(1.0 + exp(-Sum[p][j])) ;    /* Compute the sigmoid of all the elements of SumH (activation of neurons in layer 1) */
              }
              ...
              for( h=1; h<NumHL; h++) {
                for(k=1; k<=nupl[h+1]; k++) {
                  ...
                  Sum[p][k] = temp;
                  H2H[h][p][k] = 1.0/(1.0 + exp(-Sum[p][k])) ;   /* Sigmoidal Outputs *//* Compute the sigmoid of all the elements of SumO */
                }
              }
              ...
              for( k = 1 ; k <= nupl[NumHL+1] ; k++ ) {    /* compute output unit activations and errors */
                  ...
                  Output[p][k] = 1.0/(1.0 + exp(-Sum[p][k])) ;   /* Sigmoidal Outputs *//* Compute the sigmoid of all the elements of SumO */
  ...
  /*              DeltaO[p][k] = Target[p][k-1] - Output[p][k];     Linear Outputs, SSE */
        ...
        printf("Elapsed time (FEEDFORWARD - OUTPUT ERROR - BACKPROPAGATION): %lf\n",omp_get_wtime() - startime);

        /** END ITERATING THE TRAINING EXAMPLES **/

        /** GRADIENT DESCENT **/
        /** alpha is the "momentum" **/
        /** DELTA BIASES FOR OUTPUT LAYER**/

        /** DELTA WEIGHTS AND BIASES FOR OUTPUT LAYER**/
        startime = omp_get_wtime();
        #pragma omp parallel private(np,p,j,k)
        {
            int id = omp_get_thread_num(); // me
            int nthreads = omp_get_num_threads(); // how many threads
                ...
                for(k = 1; k <= nupl[NumHL+1] ; k++){
                    for(j = 0; j <= nupl[NumHL] ; j++){
                      ...
                      if(id == 0 && np == 0){ // Only one thread does that (id == 0) and at first (when np == id == 0)
                          threadDeltaHO[id][k][j] = alpha * DeltaWeightHO[k][j]; // inizialization
                      }

                      threadDeltaHO[id][k][j] += eta * ((j>0)?H2H[NumHL-1][p][j]:1) * DeltaO[p][k];
                    }
                }
        ...
        for(k = 1 ; k <= nupl[NumHL+1] ; k++){
          for(j = 0 ; j <= nupl[NumHL] ; j++){
            tempSum = 0.0;
            for(i = 0 ; i < nthreads ; i++){
              tempSum += threadDeltaHO[i][k][j];
            }
            DeltaWeightHO[k][j] = tempSum;
          }
        }
        printf("Elapsed time (DELTA WEIGHTS AND BIASES FOR OUTPUT LAYER): %lf\n",omp_get_wtime() - startime);

        startime = omp_get_wtime();

        /** DELTA WEIGHTS FOR HIDDEN LAYERS**/
        for(h=NumHL; h>1; h--) {
            for( j = 1 ; j <= nupl[h] ; j++ ) {
              for( i = 0 ; i <= nupl[h-1] ; i++ ) {
                tempSum = alpha * DeltaWeightH2H[h-1][i][j];
                for( np = 0 ; np < NumPattern ; np++ ) {
                  p = ranpat[np];
                  tempSum += eta * ((i==0)?1:H2H[h-2][p][i]) * DeltaH2H[h-1][p][j];  
        ...
        for( j = 1 ; j <= nupl[1] ; j++ ) {
          for( i = 0 ; i <= nupl[0] ; i++ ) {
            tempSum = alpha * DeltaWeightH2H[0][i][j];
            for( np = 0 ; np < NumPattern ; np++ ) {
              p = ranpat[np];
              tempSum += eta * ((i==0)?1:Input[p][i-1]) * DeltaH2H[0][p][j];  
              /* Matrix DeltaWeightIH = eta x (Trasposte of Input_Matrix) x DeltaH */
              /* DeltaWeight is initialized with the "momentum" */
            }
            DeltaWeightH2H[0][i][j] = tempSum;
          }
        }

        printf("Elapsed time (DELTA WEIGHTS AND BIASES FOR HIDDEN LAYERS): %lf\n",omp_get_wtime() - startime);

        /** UPDATE WEIGHTS AND BIASES **/
        
        for( k = 1 ; k <= NumOutput ; k++ ){/* update weights WeightHO */
          for( j = 0 ; j <= nupl[NumHL] ; j++ ){
              WeightHO[j][k] += DeltaWeightHO[k][j];
          }
        }
    ...
    for( k = 1 ; k <= NumOutput ; k ++ ) {
      ...
      fprintf(fpd,"%g ",DeltaWeightHO[k][0]);
      for( j = 1 ; j <= nupl[NumHL] ; j++ ) {
        if(verbose) {
          printf("Weight H[%d] to O[%d]: %f\n",j,k,WeightHO[j][k]);
        }
        fprintf(fp,"%7.5f ",WeightHO[j][k]);
        fprintf(fpd,"%g ",DeltaWeightHO[k][j]);
      }
      fprintf(fp,"\n");
      fprintf(fpd,"\n");
    }
